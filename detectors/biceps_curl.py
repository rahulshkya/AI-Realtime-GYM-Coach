...
class BicepsCurlDetector(BaseExercise):
    ...
    def process(self,landmarks) -> dict:
        ...
        if key_landmarks_visible:
            if elbow_angle < self.UP_THRESHOLD:
                self.stage="UP"
                if self._shoulder_X_baseline is None:
                    self._shoulder_X_baseline = landmarks[shoulder_idx].x
        ...
        elbow_drift = abs(shoulder_x - elbow_x)
        ...
        torso_angle = self.__safe_angle(dx, dy)

        if torso_angle <= self.SWING_THRESHOLD:
            swing_status=" NOTSWINGING"
    ...
    def __safe_angle(self,dx,dy):
        return math.degrees(math.atan2(abs(dy),abs(dx))) if dx != 0 or dy != 0 else 0